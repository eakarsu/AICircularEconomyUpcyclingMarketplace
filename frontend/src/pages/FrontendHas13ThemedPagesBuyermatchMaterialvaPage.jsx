...
export default function FrontendHas13ThemedPagesBuyermatchMaterialvaPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-frontend-has-13-themed-pages-buyermatch-materialva/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Frontend has 13 themed pages (BuyerMatch, MaterialValuation, PricingStrategy, etc.) but only ai.js backend</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-frontend-has-13-themed-pages-buyermatch-materialva/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
