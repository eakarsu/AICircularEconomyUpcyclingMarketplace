...
export default function AgenticUpcyclerThatWalksUsersFromWastePhotoPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-upcycler-that-walks-users-from-waste-photo/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic upcycler that walks users from waste photo → product idea → marketplace listing</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-upcycler-that-walks-users-from-waste-photo/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
