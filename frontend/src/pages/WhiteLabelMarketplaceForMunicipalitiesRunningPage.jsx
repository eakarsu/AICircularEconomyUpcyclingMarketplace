...
export default function WhiteLabelMarketplaceForMunicipalitiesRunningPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-white-label-marketplace-for-municipalities-running/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>White-label marketplace for municipalities running circular-economy programs</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-white-label-marketplace-for-municipalities-running/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
